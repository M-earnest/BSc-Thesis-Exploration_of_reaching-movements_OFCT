...
def get_descriptive_stats(subject_parameters, event_file):
    '''
    ...
    for idx, i in enumerate(event_file['result']):
    #     print(idx)
        if event_file['result'][idx] == 'success':
            sucesses.append(event_file['result'][idx])
        elif event_file['result'][idx] == 'failure':
            failures.append(event_file['result'][idx])

    num_failures = len(failures)
    num_successes = len(sucesses)
    percentage_correct = num_successes/(num_successes+num_failures)

    df_descriptives['failures'] = len(failures)
    df_descriptives['sucesses'] = len(sucesses)
    df_descriptives['percentage_correct'] = percentage_correct


    return df_descriptives

def group_by_result(event_file):
    '''
    ...
    for result, data in event_file.groupby('result'):
#         print(result)
    #     if result == 'sucess':
        if result == 'sucess':
...
# successful_trials, failed_trials = group_by_result(event_file)
...
def group_by_position(event_file):
    '''
...
# r1,r2,r3,m1,m2,m3,l1,l2,l3 = group_by_position(successful_trials)

def group_by_vertical_axis(event_file):
    '''
    ...
    try:
        ...
        print("""group not in df!""")

def group_by_horizontal_axis(event_file):
    '''
    ...
    for index, x in enumerate(event_file['box_pos_id']):
        if list(event_file['box_pos_id'][index])[1] == str(1):
            if low_boxes.empty == False:
                low_boxes = low_boxes.append(event_file.loc[index])
            elif low_boxes.empty:
                low_boxes = pd.DataFrame(event_file.loc[index]).T
        if list(event_file['box_pos_id'][index])[1] == str(2):
            if middle_boxes.empty == False:
                middle_boxes = middle_boxes.append(event_file.loc[index])
            elif middle_boxes.empty:
                middle_boxes = pd.DataFrame(event_file.loc[index]).T
        if list(event_file['box_pos_id'][index])[1] == str(3):
            if high_boxes.empty == False:
                high_boxes = high_boxes.append(event_file.loc[index])
            elif high_boxes.empty:
                high_boxes = pd.DataFrame(event_file.loc[index]).T
...
def group_by_size_and_pos(event_file):
    '''
    ...
    for size_id, data in event_file.groupby('box_size_id'):
        if size_id == 's1':
            for box_pos_id, data_ in data.groupby('box_pos_id'):
                if box_pos_id ==  'r1':
                    if s1_r1.empty:
                        s1_r1 = data_
...
def extract_and_resample(event_file, data_file, markers):
    '''
    ...
    for i in event_file['entered']:
        entered.append(i+1)
    for i in event_file['exited']:
        exited.append(i)
...
#         for j in data_file.columns[1:]:
        for j in data_file[markers]:
            ...
            if j.split('_')[2] == 'x':
                ...
                resampled = signal.resample(pad, 600)
...
#             print(str(df_dimension[i][j])+ ' - '+ str(mean_dimension[j])+' = '+str(df_dimension[i][j] - mean_dimension[j]))
        if counter_x == 0:
...
def extract_mean_factor_loading(reshaped):
    ...
    sns.barplot(x=list(range(1,len(var_explained)+1)),
            y=var_explained)
    plt.xlabel('Singulärwerte')
    plt.ylabel('Aufgeklärte Varianz(%)')
    plt.ylim(0,1)
...
def mean_trial_length(event_file, data_file, markers):
    '''
...
def plot_mean_loadings(mean_loadings):
    ...
    ax.set_title('subject x shelf x')
...
## descriptive Stats for Movement profiles
def analysis_by_result(event_file,data_file,marker,result_group, pos_of_interest, path):
    '''
    Function to analyse movement data based on result of trial.
    Links functions
        - group_by_result - group data based on results
        - group_by_position - group data based on position
    ...
    path = path + '1H/'+ 'analysis_'+result_group+'_trials/'+ (marker[0][0:9])+'/'
    ...
    if result_group == 'successful':
        ...
        print(result_group)
        r1,r2,r3,m1,m2,m3,l1,l2,l3 = group_by_position(failed_trials)

    for idx, i in enumerate(pos_of_interest):
        try:
            if i == 'r1':
                df_x, df_y, df_z = extract_and_resample(r1, data_file, marker)
                ...
                df_x, df_y, df_z = extract_and_resample(r2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(r3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(m1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(m2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(m3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(l1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(l2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(l3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
...
def analysis_pos(event_file,data_file,marker, pos_of_interest, path):
    '''
    ...
    '''
    # create subj directory
    path = path +'1H/'+ 'analysis_pos_only/' + (marker[0][0:9])+'/'
    try:
        os.makedirs(path)
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    r1,r2,r3,m1,m2,m3,l1,l2,l3 = group_by_position(event_file)
...
def analysis_example_pos(event_file,data_file,marker, pos_of_interest, path):
    '''
    Function to analyse movement data based on position of target.
    Links functions
        - group_by_position - group data based on position
    ...
    for i in pos_of_interest:
        ...
        try:
            ...
            stdm_df.to_csv(path+i+'_stdm'+'.csv', sep=',', header=False, float_format='%.2f', index=False)
            var_expl_df = pd.DataFrame(var_explained.astype(float))
            var_expl_df.to_csv(path+i+'_stdm'+'.csv', sep=',', header=False, float_format='%.2f', index=False)
            reshaped_df = pd.DataFrame(reshaped_df.astype(float))
            reshaped_df.to_csv(path+i+'_corrected_trials-xyz'+'.csv', sep=',', float_format='%.2f', index=False)
            return ML_df
...
def analysis_example_size(event_file,data_file,marker, size_of_interest, path):

    # create subj directory
    path = path +'1H/'+ 'analysis_size/' + (marker[0][0:9])+'/'
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise
    s1,s2,s3 = group_by_size(event_file)
    for i in size_of_interest:
        print(i)
        try:
            if i == 's1':
                df_x, df_y, df_z = extract_and_resample(s1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
...
def analysis_example_pos_and_size(event_file,data_file,marker, combination_of_interest,path):
    '''
    Function to analyse movement data based on position and size of target.
    Links functions
        - group_by_size_and_pos - group data based on position and size
        - get corrected - zero-center positional data
        - reshape - transform matrix to necessary format for svd
        - extract_mean_factor_loading - perform svd and extract mean factor loading of u1
        - plot_mean_loadings - plot mean factor loading
    ...
    '''
    # create subj directory
    path = path +'1H/'+ 'analysis_pos_and_size/' + (marker[0][0:9])+'/'
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise
    s1_r1,s1_r2,s1_r3,s1_m1, s1_m2,s1_m3, s1_l1,s1_l2,s1_l3, s2_r1,s2_r2,s2_r3,s2_m1,s2_m2,s2_m3, s2_l1,s2_l2,s2_l3,s3_r1,s3_r2,s3_r3,s3_m1, s3_m2,s3_m3, s3_l1,s3_l2,s3_l3 = group_by_size_and_pos(event_file)
    for i in combination_of_interest:
        print(i)
        try:
            if i == 's1_r1':
                df_x, df_y, df_z = extract_and_resample(s1_r1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_r2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_r3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_m1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_m2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_m3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_l1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_l2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s1_l3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_r1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_r2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_r3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_m1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_m2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_m3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_l1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_l2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s2_l3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_r1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_r2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_r3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_m1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_m2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_m3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_l1, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_l2, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
                ...
                df_x, df_y, df_z = extract_and_resample(s3_l3, data_file, marker)
                x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                # to csv
...
#             return x_mean_dimension, x_variance_dimension, x_df_corrected, y_mean_dimension, y_variance_dimension, y_df_corrected, z_mean_dimension, z_variance_dimension, z_df_corrected, U, S, VT, mean_loadings, stdm, concatenated
        except KeyError:
            print('passed')

def analysis_example_pos_and_size_vertical(event_file,data_file,marker, group_of_interest, size_of_interest, path):
    '''
    Function to analyse movement data based on vertical position and size of target.
    Links functions
        - group_by_size
        - group_by_vertical_axis - group data based on verical position
        - get corrected - zero-center positional data
        - reshape - transform matrix to necessary format for svd
        - extract_mean_factor_loading - perform svd and extract mean factor loading of u1
        - plot_mean_loadings - plot mean factor loading
    ...
    datafile: containing positional data of markers in x,y,z dimensions
    marker = name of marker of interest as list of strings (marker-names in data_file.columns)
    group_of_interest = abbreavtaion for name of target position of interest
                        as list of strings (e.g svertical_middle etc.)
    size_of_interest = abbreavtaion for name of size of interest as list of strings (e.g "s1", "s2" etc.)
    path = path to save data to
    ...
    '''
    # create subj directory
    path = path +'1H/'+ 'analysis_pos_and_size_vertical/' + (marker[0][0:9])+'/'
    ...
    for j in size_of_interest:
        try:

            print(size_of_interest)
            if j == 's1':
                vertical_left,vertical_middle,vertical_right = group_by_vertical_axis(s1)
            elif j == 's2':
                vertical_left,vertical_middle,vertical_right = group_by_vertical_axis(s2)
            elif j == 's3':
                vertical_left,vertical_middle,vertical_right = group_by_vertical_axis(s3)
            try:
                for idx, i in enumerate(group_of_interest):
                    print(i)
                    if i == 'vertical_left':
                        df_x, df_y, df_z = extract_and_resample(vertical_left, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                        ...
                        # to csv
                        fig = plot_mean_loadings(mean_loadings)
                    elif i == 'vertical_middle':
                        df_x, df_y, df_z = extract_and_resample(vertical_middle, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                        ...
                        # to csv
                        fig = plot_mean_loadings(mean_loadings)
                    elif i == 'vertical_right':
                        df_x, df_y, df_z = extract_and_resample(vertical_right, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                ...
                print("""passed: {size_of_interest} + {group_of_interest} -> check dataframe!""".format(size_of_interest=size_of_interest,
                                                                                 group_of_interest=group_of_interest[idx]))
        except (TypeError):
                print("""passed: {size_of_interest} -> check dataframe!""".format(size_of_interest=size_of_interest))



def analysis_example_pos_and_size_horizontal(event_file,data_file,marker, group_of_interest, size_of_interest, path):
    # create subj directory
    '''
    Function to analyse movement data based on vertical position and size of target.
    Links functions
        - group_by_horizontal_axis - group data based on horizontal position
        - get corrected - zero-center positional data
        - reshape - transform matrix to necessary format for svd
        - extract_mean_factor_loading - perform svd and extract mean factor loading of u1
        - plot_mean_loadings - plot mean factor loading
    ...
    datafile: containing positional data of markers in x,y,z dimensions
    marker = name of marker of interest as list of strings (marker-names in data_file.columns)
    group_of_interest = abbreavtaion for name of horozontal target position of interest
                        as list of strings (e.g svertical_middle etc.)
    size_of_interest = abbreavtaion for name of size of interest as list of strings (e.g "s1", "s2" etc.)
    path = path to save data to
    ...
    sample*trials as rows
    '''
    path = path +'1H/'+ 'analysis_pos_and_size_hotizontal/' + (marker[0][0:9])+'/'
    try:
        os.makedirs(path)
    except OSError as e:
        if e.errno != errno.EEXIST:
...
#             print(size_of_interest)
            if j == 's1':
                low_boxes,middle_boxes,high_boxes = group_by_horizontal_axis(s1)
            elif j == 's2':
                low_boxes,middle_boxes,high_boxes = group_by_horizontal_axis(s2)
            elif j == 's3':
                low_boxes,middle_boxes,high_boxes = group_by_horizontal_axis(s3)
            try:
                for idx, i in enumerate(group_of_interest):
                    print(i)
                    if i == 'low_boxes':
                        df_x, df_y, df_z = extract_and_resample(low_boxes, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                        ...
                        df_x, df_y, df_z = extract_and_resample(middle_boxes, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                        ...
                        df_x, df_y, df_z = extract_and_resample(high_boxes, data_file, marker)
                        x_mean_dimension, x_variance_dimension, x_df_corrected = get_corrected('x', df_x)
                        y_mean_dimension, y_variance_dimension, y_df_corrected = get_corrected('y', df_y)
                        z_mean_dimension, z_variance_dimension, z_df_corrected = get_corrected('z', df_z)
                        # to csv
                    ...
                    mean_trajectories.to_csv(path+i+'_mean_trajectories'+'.csv', sep=',',  float_format='%.2f', index=False)
                    U_df = pd.DataFrame(data=U.astype(float))
                    U_df.to_csv(path+i+'_U'+'.csv', sep=',', header=False, index=False)
                    S_df = pd.DataFrame(S.astype(float))
                    S_df.to_csv(path+i+'_S'+'.csv', sep=',', header=False, float_format='%.2f', index=False)
                    VT_df = pd.DataFrame(VT.astype(float))
                    VT_df.to_csv(path+i+'_VT'+'.csv', sep=',', header=False, float_format='%.2f', index=False)
...
#     plt.boxplot(mean_loadings)
    ...
    if p > alpha:
        print('W: '+str(s))
        print('p-value:' +str(p))
        print("""failure to reject H0: possibly drawn from normal distibution:
        ...
        print('W :'+str(s))
        print('p-value:' +str(p))
        print("""reject H0: possibly drawn from non-normal distibution:
        using wilcoxon""")
...
#         display(stats.wilcoxon(mean_loadings[40:60], mean_loadings[80:100]))
    ...
    df = pd.DataFrame({'size_a':list_size_a,'size_b':list_size_b,
                       ...
                       'marker_a':list_marker_a,'marker_a':list_marker_a,
                       'tested':list_cat,
